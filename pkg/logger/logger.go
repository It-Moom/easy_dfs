 ...
 * @Description: 日志文件
 * @Author: gabbymrh
 * @Date: 2024-07-18 10:27:24
 * @LastModifiedBy: gabbymrh
 * @LastModifiedAt: 2024-07-18 10:27:24
 */
...
// InfoString 记录一条字符串类型的 info 日志，调用示例：
//
//	logger.InfoString("SMS", "短信内容", string(result.RawResponse))
func InfoString(moduleName, name, msg string) {
	Logger.Info(moduleName, zap.String(name, msg))
}

// WarnString 记录一条字符串类型的 warn 日志，调用示例：
//
//	logger.WarnString("SMS", "短信内容", string(result.RawResponse))
func WarnString(moduleName, name, msg string) {
	Logger.Warn(moduleName, zap.String(name, msg))
}

// ErrorString 记录一条字符串类型的 error 日志，调用示例：
//
//	logger.ErrorString("SMS", "短信内容", string(result.RawResponse))
func ErrorString(moduleName, name, msg string) {
	Logger.Error(moduleName, zap.String(name, msg))
}

// FatalString 记录一条字符串类型的 fatal 日志，调用示例：
//
//	logger.FatalString("SMS", "短信内容", string(result.RawResponse))
...
// WarnJSON 记录对象类型的 warn 日志，使用 json.Marshal 进行编码。调用示例：
//
//	logger.WarnJSON("Auth", "读取登录用户", auth.CurrentUser())
...
// FatalJSON 记录对象类型的 fatal 日志，使用 json.Marshal 进行编码。调用示例：
//
//	logger.FatalJSON("Auth", "读取登录用户", auth.CurrentUser())
func FatalJSON(moduleName, name string, value interface{}) {
	Logger.Fatal(moduleName, zap.String(name, jsonString(value)))
}

// jsonString 将任意类型的数据转换为 json 字符串
...
// getLogWriter 日志记录介质。FastGo 中使用了两种介质，os.Stdout 和文件
