 ...
 * @Description: 日志配置
 * @Author: gabbymrh
 * @Date: 2024-07-18 14:38:12
 * @LastModifiedBy: gabbymrh
 * @LastModifiedAt: 2024-07-18 14:38:12
 */

package config

import "easy_dfs/pkg/config"
...
func init() {
	config.Add("log", func() map[string]interface{} {
		return map[string]interface{}{
			...
			/* ------------------ 滚动日志配置 ------------------ */
			// 日志文件路径
			"filename": config.Env("log.name", "logs/logs.log"),
			// 每个日志文件保存的最大尺寸 单位：M
			"max_size": config.Env("log.max-size", 64),
			// 最多保存日志文件数，0 为不限，MaxAge 到了还是会删
			"max_backup": config.Env("log.max-backup", 5),
			// 最多保存多少天，7 表示一周前的日志会被删除，0 表示不删
			"max_age": config.Env("log.max-age", 30),
